...
public class SakuraEntityListener implements Listener {
	...
	public void onEntityDamageByEntity(EntityDamageByEntityEvent event){
		...
		/* 他人の飼いMOBが攻撃を受けたら警告 */
		if ((ent.getType() == EntityType.OCELOT || ent.getType() == EntityType.WOLF) &&
		...
		/* デバッグ用 */
		if (false && damager.getType() == EntityType.PLAYER &&
				(OfflinePlayer) damager == Bukkit.getOfflinePlayer("syamn")){
	...
	public void targetCancelOnGoldEquipment(EntityTargetLivingEntityEvent event){
		...
		// 攻撃者がプレイヤー以外ならリターン
		if (targetEnt.getType() != EntityType.PLAYER){
			return;
		}

		// 誰かから攻撃されたMOBならリターン
		if (ent.getLastDamageCause() != null){
			if(ent.getLastDamageCause().getCause().equals(DamageCause.ENTITY_ATTACK)){
				return;
			}
		}
	...
	public void onPlayerDeath(PlayerDeathEvent event){
		...
		if (deader.getKiller() != null){
			...
			// 住民以上でない場合は申告不可能
			if (!deader.hasPermission("sakuraserver.citizen")){
				return;
			}
			// ヘルパー以上に攻撃された場合は申告不可能
			if (killer.hasPermission("sakuraserver.helper")){
				return;
			}
